...
public:
	...
	Graphic(int *handle, Vector2 pos = Vector2(), bool centerPosFlg = false, bool trans = true, int fade = 256, Vector2 zoom = Vector2(100, 100), double angle = 0.0);	// 読み込み（既存ハンドルの使用）
	~Graphic();	// 削除
	...
	static int* CreateHandle(const TCHAR *path);	// ハンドル番号を生成する（単体読み込み）
	static int* CreateHandle(const TCHAR *path, Vector2 oneSize, Vector2 divNum);	// ハンドル番号を生成する（分割読み込み）
