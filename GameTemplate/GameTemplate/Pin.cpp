...
void Pin::Update(){
	...
	case State::motions:
		...
		if (motion.IsFinished()){
			...
			pic["ぴん"]->SetZoom(motion.GetValue());
		}

		curve.Update();
		pic["ぴん"]->pos = Vector2d(GetStateTime(), curve.GetValue());
		if (curve.IsFinished()){
			pic["ぴん"]->SetColor(255, 255, 255);
		}
		else if (curve.PassedVertex()){
			pic["ぴん"]->SetColor(0, 255, 255);
		}
		else{
			pic["ぴん"]->SetColor(0, 0, 255);
		}
